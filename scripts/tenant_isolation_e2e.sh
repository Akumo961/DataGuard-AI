...
on_error() {
  local code=$?
  printf 'Tenant isolation E2E failed at line %s (exit %s).\n' "$1" "$code" >&2
  docker compose logs --no-color api document-worker >&2 || true
  exit "$code"
}
...
A="$(jwt 00000000-0000-0000-0000-000000000042 tenant-a privacy_officer)"
B="$(jwt 00000000-0000-0000-0000-000000000043 tenant-b privacy_officer)"
AUTH_A=(-H "Authorization: Bearer $A")
AUTH_B=(-H "Authorization: Bearer $B")
JSON=(-H 'Content-Type: application/json')

analysis="$(curl --fail-with-body --silent --show-error "${AUTH_A[@]}" "${JSON[@]}" -X POST "$base/api/v1/analyze" -d '{"text":"alice@example.com","purpose_defined":true,"encrypted_at_rest":true,"access_scope":"internal","data_location":"quebec"}')"
...
findings="$(curl --fail-with-body --silent --show-error "${AUTH_A[@]}" "$base/api/v1/findings?analysis_id=$analysis_id")"
finding_id="$(python -c 'import json,sys; x=json.loads(sys.stdin.read()); assert x, "No findings returned"; print(x[0]["id"])' <<<"$findings")"
test -n "$finding_id"
...
pia="$(curl --fail-with-body --silent --show-error "${AUTH_A[@]}" "${JSON[@]}" -X POST "$base/api/v1/pias" -d '{"project_name":"Tenant A PIA","data_subjects":["employees"],"purposes":["testing"],"vendors":["synthetic"],"jurisdictions":["CA-QC"],"safeguards":["encryption"]}')"
...
curl --fail-with-body --silent --show-error "${AUTH_A[@]}" "${JSON[@]}" -X POST "$base/api/v1/pias/$pia_id/transition" -d '{"target":"IN_REVIEW","reason":"Review started"}' >/dev/null
curl --fail-with-body --silent --show-error "${AUTH_A[@]}" "${JSON[@]}" -X POST "$base/api/v1/pias/$pia_id/transition" -d '{"target":"APPROVED","reason":"Approved after documented privacy review"}' >/dev/null

action="$(curl --fail-with-body --silent --show-error "${AUTH_A[@]}" "${JSON[@]}" -X POST "$base/api/v1/remediations" -d '{"title":"Tenant A remediation","description":"Synthetic remediation","priority":"HIGH","sla_hours":24}')"
remediation_id="$(python -c 'import json,sys; print(json.loads(sys.stdin.read())["id"])' <<<"$action")"
test -n "$remediation_id"
