"""Rebuild audit hashes after making persisted event IDs deterministic."""
        ...
        for row in rows:
            ...
            bind.execute(
                text(
                    ...
                    "integrity_hash = :integrity_hash WHERE id = :id"
                ),
                {
                    "id": row["id"],
                    "previous_hash": previous_hash,
...
def downgrade() -> None:
    raise RuntimeError("Audit hash-chain rebuild is irreversible")
