...
def test_person_detected_without_field_label() -> None:
    ...
    assert any(d.pii_type is PIIType.PERSON and "Jean Tremblay" in d.value for d in detections)
...
def test_address_detected_without_field_label() -> None:
    ...
    assert any(d.pii_type is PIIType.ADDRESS and "123 Rue Sainte-Catherine" in d.value for d in detections)
