...
def test_locally_issued_access_token_has_unique_jti(monkeypatch) -> None:
    ...
    first = create_access_token(subject_id="user-1", organization_id="org-1", roles={Role.ANALYST})
    second = create_access_token(subject_id="user-1", organization_id="org-1", roles={Role.ANALYST})
