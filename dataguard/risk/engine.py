...
class RiskEngine:
    ...
    _sensitivity = {
        ...
        "DATE_OF_BIRTH": 40,
        "GOVERNMENT_ID": 60,
        "HEALTH_INSURANCE_ID": 85,
        "PASSPORT": 65,
        "DRIVER_LICENSE": 60,
        "HEALTH_INFORMATION": 80,
        "FINANCIAL_INFORMATION": 65,
        "BANK_ACCOUNT": 75,
        "CREDIT_CARD": 80,
        "IP_ADDRESS": 15,
        "LOCATION": 35,
        ...
        "CUSTOMER_ID": 30,
        "TAX_ID": 65,
        "SOCIAL_INSURANCE_NUMBER": 85,
        "BIOMETRIC_DATA": 90,
        "OTHER_SENSITIVE_INFORMATION": 70,
    }

    def assess(
        self, detections: list[Detection], context: RiskContext | None = None
    ) -> RiskAssessment:
        ...
        if not detections:
            return RiskAssessment(
                ...
                RiskLevel.LOW,
                (),
                "No PII detections were supplied; this is not proof that no personal information exists.",
                (),
            )
            ...
            [
                {
                    "name": "sensitivity",
                    "points": base,
                    "detail": "highest detected sensitivity class",
                },
                {
                    "name": "quantity",
                ...
                },
                {
                    ...
                    "detail": f"mean detector confidence={confidence:.3f}",
                },
            ]
        )
        ...
        if ctx.retention_days is not None and ctx.retention_days > 365:
            ...
            factors.append(
                {
                    "name": "retention",
                    "points": min(8, (ctx.retention_days - 365) / 180),
                    "detail": f"retention_days={ctx.retention_days}",
                }
            )
        ...
        if not ctx.purpose_defined:
            score += 5
            factors.append(
                {"name": "purpose", "points": 5, "detail": "processing purpose not confirmed"}
            )
    ...
    def _recommendations(
        self, detections: list[Detection], ctx: RiskContext, level: RiskLevel
    ) -> list[str]:
        recommendations: list[str] = []
        if level in {RiskLevel.HIGH, RiskLevel.CRITICAL}:
            recommendations.append(
                "Require human privacy/security review before relying on this assessment."
            )
        if not ctx.encrypted_at_rest:
        ...
        if any(
            ...
            recommendations.append(
                "Apply enhanced safeguards and specialist review for highly sensitive identifiers/information."
            )
