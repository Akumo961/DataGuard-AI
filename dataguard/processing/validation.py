...
class DocumentValidator:
    ALLOWED = {
        ...
        ".tif": (DocumentType.IMAGE, (b"II*\x00", b"MM\x00*")),
        ".tiff": (DocumentType.IMAGE, (b"II*\x00", b"MM\x00*")),
    }
    ...
    def validate(self, document: DocumentInput) -> DocumentType:
        ...
        if magic is not None:
            signatures = magic if isinstance(magic, tuple) else (magic,)
            if not any(document.content.startswith(signature) for signature in signatures):
        ...
        if document.declared_mime:
            if self.allowed_mime_types and document.declared_mime not in self.allowed_mime_types:
                raise UnsafeDocumentError("declared MIME type is not allowed")
