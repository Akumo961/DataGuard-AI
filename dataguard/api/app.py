...
def _analysis_payload(detections, classification, risk, governance):
    return {
        "detections": [
            {"type": d.pii_type.value, "start": d.start, "end": d.end, "confidence": d.confidence, "detector": d.detector, "redacted_value": _redact(d.value)}
            for d in detections
        ],
        "classification": {"label": classification.label, "confidence": classification.confidence, "rationale": classification.rationale, "model_version": classification.model_version},
        "risk": {"score": risk.score, "level": risk.level.value, "factors": list(risk.factors), "explanation": risk.explanation, "recommendations": list(risk.recommendations)},
...
async def _persist_analysis(session: AsyncSession, principal: Principal, source_type: str, source_ref: str | None, payload: dict) -> Analysis:
    analysis = Analysis(organization_id=UUID(principal.organization_id), source_type=source_type, source_ref=source_ref, status="COMPLETED", result=payload)
    session.add(analysis)
    await session.flush()
    classification = payload["classification"]
    session.add_all(
        Finding(
            organization_id=UUID(principal.organization_id), analysis_id=analysis.id, pii_type=item["type"],
            start_offset=item["start"], end_offset=item["end"], confidence=item["confidence"], detector=item["detector"],
            classification_label=classification["label"], classification_confidence=classification["confidence"], status="OPEN",
            owner_id=principal.subject, evidence={"redacted_value": item["redacted_value"], "detector": item["detector"]},
        ) for item in payload["detections"]
    )
    session.add(AuditEvent(
        organization_id=UUID(principal.organization_id), actor_id=principal.subject, action="ANALYSIS_COMPLETED",
...
def create_app() -> FastAPI:
    ...
    async def analyze(request: AnalyzeRequest, principal: Principal = Depends(require_permission("analysis:write")), session: AsyncSession = Depends(get_session)) -> AnalyzeResponse:
        ...
        classification = classifier.classify(request.text, detections, {"organization_id": principal.organization_id, "framework": request.framework})
        risk = risk_engine.assess(detections, RiskContext(data_location=request.data_location, access_scope=request.access_scope, retention_days=request.retention_days, encrypted_at_rest=request.encrypted_at_rest, purpose_defined=request.purpose_defined, exposure=request.exposure, framework=request.framework))
        ...
        return AnalyzeResponse(analysis_id=analysis_id, organization_id=principal.organization_id,
            ...
            classification=ClassificationResponse(label=classification.label, confidence=classification.confidence, rationale=classification.rationale, model_version=classification.model_version),
    ...
    async def analyze_document(file: UploadFile = File(...), principal: Principal = Depends(require_permission("analysis:write")), session: AsyncSession = Depends(get_session)) -> AnalyzeResponse:
        ...
        classification = classifier.classify(extracted.text, detections, {"organization_id": principal.organization_id, "document_type": extracted.document_type.value})
        ...
        payload = _analysis_payload(detections, classification, risk, governance)
        payload["document"] = {"filename": extracted.filename, "document_type": extracted.document_type.value, "page_count": extracted.page_count, "warnings": list(extracted.warnings)}
        analysis_id = None
        if settings.environment != "test":
            await _tenant_session(session, principal.organization_id)
    ...
    async def list_findings(principal: Principal = Depends(require_permission("finding:read")), session: AsyncSession = Depends(get_session), pii_type: str | None = Query(default=None, max_length=80), status: str | None = Query(default=None, max_length=40), limit: int = Query(default=50, ge=1, le=100), offset: int = Query(default=0, ge=0)) -> list[FindingResponse]:
        await _tenant_session(session, principal.organization_id)
        query = select(Finding).where(Finding.organization_id == UUID(principal.organization_id))
        if pii_type: query = query.where(Finding.pii_type == pii_type)
        if status: query = query.where(Finding.status == status)
        rows = (await session.execute(query.order_by(Finding.created_at.desc(), Finding.id.desc()).offset(offset).limit(limit))).scalars().all()
        return [FindingResponse(id=str(r.id), analysis_id=str(r.analysis_id), pii_type=r.pii_type, start_offset=r.start_offset, end_offset=r.end_offset, confidence=r.confidence, detector=r.detector, classification_label=r.classification_label, classification_confidence=r.classification_confidence, status=r.status, owner_id=r.owner_id, evidence=r.evidence) for r in rows]
    ...
    async def create_pia(request: PIARequest, principal: Principal = Depends(require_permission("pia:manage")), session: AsyncSession = Depends(get_session)):
        ...
        session.add(AuditEvent(organization_id=UUID(principal.organization_id), actor_id=principal.subject, action="PIA_CREATED", object_type="pia", object_id=str(row.id), previous_state=None, new_state={"status": "DRAFT"}, request_id=None, ip_address=None, result="SUCCESS", occurred_at=datetime.now(timezone.utc)))
        await session.commit()
    ...
    async def create_remediation(request: RemediationRequest, principal: Principal = Depends(require_permission("analysis:write")), session: AsyncSession = Depends(get_session)):
        ...
        session.add(row); await session.flush()
        session.add(AuditEvent(organization_id=UUID(principal.organization_id), actor_id=principal.subject, action="REMEDIATION_CREATED", object_type="remediation", object_id=str(row.id), previous_state=None, new_state={"status": "OPEN", "priority": row.priority}, request_id=None, ip_address=None, result="SUCCESS", occurred_at=datetime.now(timezone.utc)))
        await session.commit()
